...
classifier, bounds the system: on the worst pump its recall ceiling is 0.52. We
discuss the operating-point method critically, including a naive form of it that
buys nothing.
\end{abstract}
...
\textbf{The better lever is one we could not test.} Requiring a fault to persist
across $k$ of the last $n$ windows should dominate sparse sampling, since it uses
all the data while still spending the budget once. We cannot evaluate it. ESPset
records are independent measurements with no acquisition timestamps, so any rolling
rule over a ``trailing window'' would measure the sort order of the file rather
than the physics. An earlier version of our analysis reported such numbers; they
were an artefact and were withdrawn. Testing persistence requires timestamped
acquisitions, and we flag it as the most promising untested extension of this work.
