...
\title{Normalisation Choice Outweighs Model Choice in\\
Cross-Machine Rotating-Machinery Fault Diagnosis}
...
\begin{abstract}
Cross-machine fault diagnosis requires two decisions that are usually made
silently: how to split the data, and how to normalise it. We show the second
matters more than the choice of model. On eleven in-service submersible pumps
under leave-one-machine-out, standardising each machine by its own statistics
--- common practice, and the option that uses \emph{more} information about the
test machine --- costs 0.20--0.28 macro-F1 against pooling the training machines,
on 11 of 11 machines. For comparison, the gap between a prior-fitted tabular
foundation model and a nested-tuned gradient-boosted baseline on the same data is
0.072. The effect replicates on an independent dataset from a different
laboratory, machine type and sensor suite, where per-unit normalisation drives
logistic regression exactly to the majority-class floor. \textbf{We do not
establish the mechanism}: a class-imbalance explanation is refuted by our own
controlled test, and a competing account based on absolute signal level is only
partially supported.

We situate this in a two-tier monitoring architecture for irrigation pumps, in
which battery-powered nodes perform continuous gating and a local dry-run trip
while a shared ARM gateway classifies escalated events, and report a five-rung
leakage ladder under which random-window splits nearly double reported macro-F1
on real machines. We also report that the gate, not the classifier, bounds the
system: on the worst pump its recall ceiling is 0.52.
\end{abstract}
...
\textbf{Contributions.}
(C1) \textbf{The main result}: on cross-machine fault diagnosis, normalisation
strategy outweighs model choice --- 0.20--0.28 macro-F1 on eleven in-service
pumps, against a 0.072 margin between models, on 11 of 11 machines. Replicated on
an independent bearing benchmark. The mechanism is \emph{not} established, and we
refute our own first explanation.
(C2) A five-rung leakage ladder quantifying how much reported accuracy is
...
(C3) A two-tier architecture with gateway latency measured on the deployment
board and a gate escalation rate measured on eleven in-service pumps --- including
the finding that the gate, not the classifier, bounds end-to-end recall.
(C4) Evidence on whether reference-set substitution earns its cost, at a
...
\textbf{The mechanism is unresolved.} We report an effect, not an explanation.
...
\section{Conclusion}
On cross-machine rotating-machinery fault diagnosis, the choice of normalisation
strategy is worth more than the choice of model. Standardising each machine by its
own statistics --- standard practice, and the option using more information about
the test machine --- costs 0.20--0.28 macro-F1 on eleven in-service pumps against
pooling the training machines, on every machine without exception, where the
margin between competing models on the same data is 0.072. The effect replicates
on an independent bearing benchmark, where it drives a linear model exactly to the
majority-class floor. We could not establish why, and we refute our own first
explanation rather than publish it.

The practical consequence is small and concrete: report the normalisation strategy
with every cross-machine result, because two papers using different strategies are
not comparable, and the difference will usually exceed whatever modelling
contribution is being claimed.
...
Transmission is not the node's energy problem; continuous sensing is. And the
gate, not the classifier, bounds what the system can detect.
