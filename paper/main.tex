...
inflation is \emph{smallest} on synthetic data, where practitioners are most
likely to look for it. Finally we report that the gate's recall ceiling, not the
...
Table~\ref{tab:leakage} gives the headline protocol result. \textbf{The effect
grows with how real the data is.} On synthetic signals whose fault signatures
were written into the generator by hand, the split barely matters --- the
signature is in every window and any split recovers it. On in-service pumps it
nearly doubles the reported score. This ordering is itself the finding: a
practitioner validating on simulated or single-session data will not observe the
problem that dominates their field deployment. Our 1.9$\times$ on ESPset is
consistent in direction with the 1.21$\times$ that~\cite{varejao2024} report on
the same data from removing similarity bias, and larger, as the stricter rung
predicts.
...
\midrule
Synthetic & 1.000 & 0.930 (LOMO) & $1.1\times$ \\
ESPset (11 pumps) & 0.793 & 0.425 (LOMO) & $\mathbf{1.9\times}$ \\
Twente (rig) & 0.853 & 0.352 (record) & $\mathbf{2.4\times}$ \\
\bottomrule
\end{tabular}
\end{table}
